...
//topNouns1 = topNouns.slice(0, 10);
//topNouns4 = topNouns.slice(topNouns.length-10, topNouns.length-0);
//
//
//topNouns = topNouns1.concat(topNouns4);
//console.log("CRITICAL");
//console.log(topNouns4.length);
//console.log(topNouns1.length);
...
for(i = 0; i<continuations.length; i++) {
	...
	if(conditionAssignment[i] == 0) {
		...
		for(j = 0; j<words2.length; j++) {
			if(j < regions.length && regions[j] == "REGION_3_0") {
				if(Math.random() > 0.5) {
		...
		for(j = 0; j<words2.length; j++) {
			...
			} else if(j >= words1.length || words1[j] != words2[j]) {
				distractors.push(distractors2[j]);
			} else if(words2[j] == words1[j]) {
				distractors.push(distractors1[j]);
			}
		}
		item.a = distractors.join(" ");
