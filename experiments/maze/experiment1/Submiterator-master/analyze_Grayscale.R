...
dataPlot = ggplot(predictions, aes(x=(EmbeddingBias), y=(RTPred), group=paste(HasSC, HasRC), linetype=paste(HasSC, HasRC))) + geom_smooth(method="lm") + geom_point(data=data %>% filter(Region == "REGION_3_0") %>% group_by(HasSC, HasRC, noun, EmbeddingBias) %>% summarise(rt=mean(exp(LogRT))), aes(x=EmbeddingBias, y=rt, group=paste(HasSC, HasRC)), alpha=0.1) +  scale_linetype_manual(values = c("FALSE FALSE" = "dotdash", "TRUE FALSE"="dashed", "TRUE TRUE"="solid")) + theme_bw() + theme(legend.position="none") + xlab("Embedding Bias") + ylab("Reading Time (milliseconds)") + geom_errorbar(data=predictionsPoints, aes(x=EmbeddingBias, ymin=exp(lower), ymax=exp(upper)), color="blue", alpha=0.8, width=0.3) + ylim(800, 1600)
ggsave(dataPlot, file="figures/logRT-points-fit_errorbars_noLogTransform_Greyscale.pdf", height=3.5, width=1.8)
