...
//      document.getElementById("completion").value = "";
     ...
     var radio_button_list = document.getElementsByName('rating_understand');
     console.log(radio_button_list);
     var radio_button;
     var count;
     for(count = 0; count<radio_button_list.length; count++) {
    ...
    button : function() {
      ...
      for(count = 0; count<radio_button_list.length; count++) {
        if(radio_button_list[count].checked) {
 	       hasValue_understand= true;
        }
      };

	hasValue_response = false;
     var radio_button_list = document.getElementsByName('response');
     console.log(radio_button_list);
     var radio_button;
     var count;
     ...
     for(count = 0; count<radio_button_list.length; count++) {
       if(radio_button_list[count].checked) {
	       hasValue_response= true;
	       response = radio_button_list[count].value;
       }
     };

console.log(document.getElementById("wrong").style.display == "none");
console.log(document.getElementById("wrong").style.display);

      if(!hasValue_understand | !hasValue_gramm |!hasValue_response) {
         $(".err").show();
      } else if(response != this.stim.answer && document.getElementById("wrong").style.display == "none") {
         $(".wrong").show();
    ...
    log_responses : function() {
	   ...
	   if(this.stim.question != null) {
        ...
        exp.data_trials.push({
          ...
          "given_answer" : answer,
          "rating_understand" : document.querySelector('input[name="rating_understand"]:checked').value,
//          "rating_gramm" : document.querySelector('input[name="rating_gramm"]:checked').value,
