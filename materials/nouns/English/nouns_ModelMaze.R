...
for(i in 1:nrow(nouns)) {
   joint = round((nouns$CountThat[[i]]))
   unigram = round((nouns$CountBare[[i]]))
   if(unigram == 0) {
    CILower = c(CILower, NA)
    CIUpper = c(CIUpper, 0)
   } else{
...
nouns$Conditional = log(nouns$CountThat) - log(nouns$CountBare)
...
ggsave(plot, file="figures/All_nouns_byType.pdf", height=12, width=6)


nouns[order(nouns$Conditional),]

write.table(nouns[order(nouns$Conditional),], file="output/All_nouns_byType.tsv", sep="\t", quote=F, row.names=F)
