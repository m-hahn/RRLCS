...
nouns_counts = read.csv("../corpus_counts/ukwac/results/results_counts4.py.tsv", sep="\t")
...
library(ggrepel)
plot = ggplot(nouns, aes(x=Conditional, y=Noun, color=Group, group=Group)) + geom_point() + geom_errorbarh(aes(xmax=CIUpper, xmin=CILower, color=Group)) + geom_text_repel(aes(label=Noun, fontface=InMaze)) + theme_bw() + theme(axis.text.x=element_text(angle = 90)) + xlab("Embedding Bias (ukWaC)") + ylab(NULL)
ggsave(plot, file="figures/All_nouns_byType_ukWaC.pdf", height=12, width=6)


nouns[order(nouns$Conditional),]

write.table(nouns[order(nouns$Conditional),], file="output/All_nouns_byType_ukWaC.tsv", sep="\t", quote=F, row.names=F)
