library(tidyr)
library(dplyr)
data_E0 = read.csv("/juice/scr/mhahn/reinforce-logs-both-short/full-logs-tsv-perItem/collect12_NormJudg_Short_Cond_W_GPT2_ByTrial_E0.py.tsv", sep="\t") %>% rename(SurprisalsWithThat=S1, SurprisalsWithoutThat=S2) %>% filter(grepl("_TPL", Script))
...
data_E2 = read.csv("/juice/scr/mhahn/reinforce-logs-both-short/full-logs-tsv-perItem/collect12_NormJudg_Short_Cond_W_GPT2_ByTrial_VN3.py.tsv", sep="\t") %>% rename(SurprisalsWithThat=S1, SurprisalsWithoutThat=S2) %>% filter(!grepl("245_", Item)) %>% filter(grepl("_TPL", Script))
...
for(delta in c(0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95 )) {
 for(lambda in c(0.0, 0.25, 0.5,  0.75, 1)) {
  outFileName = paste("slopes/", "FIXED_analyze_inter_Fives_Simple_OnlyTPL_WithOne.R_VN5_", delta, "_", lambda, ".tsv", sep="")
#  if(!file.exists(outFileName)) {
   cat(delta, " ", lambda, "\n")
   data2 = data %>% filter(deletion_rate==delta, predictability_weight==lambda) %>% group_by(Noun, compatible.C, EmbBias.C, Item, SC.C, RC.C) %>% summarise(SurprisalReweighted=median(SurprisalReweighted))
   if(nrow(data2) > 0) {
#     contrasts(data2$Item) = contr.sum(length(levels(data2$Item)))
     model = lm(SurprisalReweighted ~ RC.C * compatible.C * Item + EmbBias.C*RC.C * Item + EmbBias.C*compatible.C*Item + SC.C*EmbBias.C*Item, data=data2)
