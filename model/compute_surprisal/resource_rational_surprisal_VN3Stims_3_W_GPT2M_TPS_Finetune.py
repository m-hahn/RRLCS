...
parser.add_argument("--learning_rate_lm", type = float, default= float('nan')) # 0.0001, 
...
relevantPath = glob.glob("../data/pred_1.0_del_*_full/char-lm-ud-stationary_12_SuperLong_WithAutoencoder_WithEx_Samples_Short_Combination_Subseq_VeryLong_WithSurp12_NormJudg_Short_Cond_Shift_NoComma_Bugfix_VN3Stims_3_W_GPT2M_TPS.py_"+args.load_from_joint+".model")[0]
...
#     #            quit()
              ...
              print("\t".join([str(w) for w in [(list(test_df["Label"])[sentenceID]), (list(test_df["critical_word"])[sentenceID]), round(float( surprisalOfNextWord),3), round(float( reweightedSurprisalsMean),3),  (list(test_df["Sentence"])[sentenceID])]]), file=outFile)
...
#      assert False
      ...
      print("LOSS", loss, counter, [x[0]/x[1] for x in averageLossOverEpoch])
