...
checkpoint = torch.load(glob.glob("CODEBOOKS_MEMORY/*"+str(args.load_from_joint)+"*")[0])
...
if True or args.load_from_autoencoder is not None:
  ...
  checkpoint_ = torch.load("CODEBOOKS/"+args.language+"_"+"autoencoder2_mlp_bidir_Erasure_SelectiveLoss.py"+"_code_"+str(checkpoint["arguments"].load_from_autoencoder)+".txt")
...
if True or args.load_from_lm is not None:
  ...
  checkpoint_ = torch.load("CODEBOOKS/"+args.language+"_"+lm_file+"_code_"+str(checkpoint["arguments"].load_from_lm)+".txt")
  for i in range(len(checkpoint_["components"])):
